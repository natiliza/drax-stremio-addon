...
// ==== EMBY CONFIG: fill once and leave it ====
// Example:
// const EMBY_BASE_URL = 'http://192.168.1.50:8096'.replace(/\/$/, '');
// const EMBY_USER_ID = 'YOUR_EMBY_USER_ID';
// const EMBY_TOKEN = 'YOUR_EMBY_TOKEN';
...
const QUEUE_CATEGORIES = [
  { catalogId: 'translations_in_progress', type: 'series', name: 'תרגומים בתהליך', mode: 'translationQueue', queueFilter: 'active' },
  { catalogId: 'translations_completed', type: 'series', name: 'תרגומים שהסתיימו', mode: 'translationQueue', queueFilter: 'done' },
  { catalogId: 'translations_ollama_library', type: 'series', name: 'תרגומי Ollama', mode: 'translationQueue', queueFilter: 'all' }
];
...
async function resolveSubtitleContext(type, rawId) {
  ...
  if (type === 'movie') {
    if (isDirectImdb) {
      return {
        type: 'movie',
        upstreamType: 'movie',
        upstreamId: incomingId,
        imdbId: incomingId,
        title: incomingId,
        year: '',
        displayName: incomingId
      };
    }

    const itemId = stripIdPrefix(incomingId);
  ...
  const afterPrefix = isDirectImdb ? incomingId : stripIdPrefix(incomingId);
  const pieces = String(afterPrefix).split(':');
  const seriesRoot = pieces[0];
  const season = Number(pieces[1] || 0);
  const episode = Number(pieces[2] || 0);
  if (!seriesRoot || !season || !episode) return null;

  if (isDirectImdb) {
    const label = `${seriesRoot} S${String(season).padStart(2, '0')}E${String(episode).padStart(2, '0')}`;
    return {
      type: 'series',
      upstreamType: 'series',
      upstreamId: `${seriesRoot}:${season}:${episode}`,
      imdbId: seriesRoot,
      season,
      episode,
      title: seriesRoot,
  ...
  const seriesItem = await getSeriesById(seriesRoot);
...
function buildQueueMeta(queueId) {
  ...
  if (!job) return { meta: { id: `transjob_${queueId}`, type: 'series', name: 'תרגום לא נמצא', videos: [] } };
  return {
    meta: {
      id: `transjob_${job.queueId}`,
      type: 'series',
      ...
      if (!isHebrewish(item.lang)) {
        ...
        if (!seen.has(translatedKey)) {
          ...
          subtitles.push({
            ...
            lang: `תרגום לעברית (Ollama) · ${displayLang(item.lang)} · ${provider.name}`
          });
        }
      }
    }
