...
const manifest = {
  id: 'local.drax.hebrew.emby.v055',
  version: '0.5.5',
  name: 'DRAX Emby עברית FIX 5',
...
function itemMatchesSearchTerm(item, search) {
  const q = normSearchText(search);
  if (!q) return true;
  const fields = [item && item.Name, item && item.OriginalTitle, item && item.SortName, item && item.ForcedSortName, item && item.Overview, item && item.ProductionYear];
  const hay = normSearchText(fields.filter(Boolean).join(" "));
  if (!hay) return false;
  if (hay.includes(q)) return true;
  const tokens = q.split(" ").filter((x) => x.length >= 2);
  return tokens.length > 0 && tokens.every((t) => hay.includes(t));
}
function strictFilterSearchResults(items, search) {
  const term = String(search || "").trim();
  if (!term) return items || [];
  return (items || []).filter((item) => itemMatchesSearchTerm(item, term));
}

async function searchCategoryOnly(category, search, base, skip = 0, limit = 50) {
  const term = String(search || '').trim();
  if (!term) return { metas: [] };

  // Search is scoped to the current catalog/category only.
  // If this category has no real match, return empty.
  // Never fall back to latest/default content during search.
  const itemType = category.type === 'movie'
    ? (category.itemType || 'Movie')
    : (category.itemType || 'Series');
  ...
  if (category.years) params.Years = category.years;

  try {
    const data = await embyFetch(`/emby/Users/${EMBY_USER_ID}/Items`, params);
    const items = strictFilterSearchResults(uniqueById(filterLooseItems(normalizeArrayResponse(data), itemType)), term);
    debug('category search result', category.catalogId, term, items.length);
    const metas = items.map((item) => category.type === 'movie' ? toMovieMetaPreview(item, base) : toSeriesMetaPreview(item, base));
    return { metas };
  } catch (e) {
    debug('category search failed', category.catalogId, term, String(e));
    return { metas: [] };
  }
}
...
async function searchAllByType(type, search, base, skip = 0, limit = 50) {
  ...
  const data = await embyFetch(`/emby/Users/${EMBY_USER_ID}/Items`, params);
  let items = strictFilterSearchResults(uniqueById(filterLooseItems(normalizeArrayResponse(data), itemType)), search);
...
async function buildCatalog(category, base, extra = {}) {
  ...
  if (search) {
    return searchCategoryOnly(category, search, base, skip, 50);
  }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (parts[0] === 'catalog' && parts.length >= 3) {
      ...
      const extraFromPath = parts.length > 3 ? parseExtra(parts.slice(3).join("/")) : {};
